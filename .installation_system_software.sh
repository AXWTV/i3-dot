...
if [ "$DISTRO" == "debian" ] || [ "$DISTRO" == "ubuntu" ]; then
    ...
    sudo apt install -y cmake meson pkg-config asciidoc curl neofetch rofi-dev autoconf automake libtool-bin libtool
    # flatpack flathub
    ...
    sudo pacman -S i3-gaps polybar alacritty neofetch rofi btop pavucontrol dunst zsh lsd feh curl vim nvim thunar ranger bluez bluez-utils blueman gtk-engine-murrine rofi-emoji && yay -S picom-git eww
    # flatpack flathub
    sudo packman -S flatpak
    sudo flatpak remote-add --if-not-exists flathub https://flathub.org/repo/flathub.flatpakrepo
    #zscroll
    git clone https://github.com/noctuid/zscroll
    cd zscroll
